...
import { saveRole, deleteRole, getRoles } from "../../actions/roleActions.js";
import { validateRequiredFields } from "../../utilities/validator.js";
...
export class ManageRoles extends React.Component {
  ...
  deleteRole(id) {
    this.props.deleteRole(id).then(() => {
      this.props.getRoles().then(() => Materialize.toast("Role deleted", 2000));
    });
  }
  ...
  onChange(event, type) {
    if (type === "new")
      return this.setState({ newRole: { name: event.target.value } });
    return this.setState({
      editRole: Object.assign({}, this.state.editRole, {
  ...
  onSave(event, type) {
    ...
    const { valid, errors } = validateRequiredFields(
      [this.state.editRole.name],
      ["name"],
    );

    if (valid) {
      return this.props
        .saveRole(this.state.editRole)
        .then(() => {
